    ...
    public function createSupply(Request $req)
    {
        ...
        if ($check_access->kelola_barang == 1 && $supply_system->status == true) {
            ...
            foreach ($req->kode_barang_supply as $no => $kode_barang) {
                ...
                // Hitung total pengeluaran (harga beli x jumlah untuk semua item)
                $total_pengeluaran = 0;
                foreach ($req->harga_beli_supply as $index => $harga) {
                    $jumlah = $req->jumlah_supply[$index];
                    $total_pengeluaran += $harga * $jumlah;
                }
                ...
                Finance::create([
                    ...
                    'keterangan' => 'Supply barang oleh ' . Auth::user()->nama
                ]);
            }
